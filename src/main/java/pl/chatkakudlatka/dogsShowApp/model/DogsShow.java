...
public class DogsShow {
    public enum DogClass {
        ...
        private boolean isBitch;

        private DogClass(String name, boolean isBitch) {
        ...
        public List<DogClass> getByGender(boolean isBitch) {
            DogClass[] allDogClasses = DogClass.values();
            List<DogClass> returnList = new ArrayList<>();
            for (int i = 0; i < allDogClasses.length; i++) {
                DogClass currentDogClass = allDogClasses[i];
